...
'''
original_file = "P.patens.V6_filtered_cosmoss_protein_codons.fas"
corrected_file = "ppa_aa_conekt.fa"
#original_file = "P.patens.V6_filtered_cosmoss_CDS_codons.fas"
#corrected_file = "ppa_conekt.fa"
...
# Checks if all genes start with 'Pp' => yes it does
with open(original_file) as orig:
    records = SeqIO.parse(original_file, 'fasta')
    for record in records:
        if record.id.startswith('Pp'):
            continue
        else:
            raise ValueError('Does not start with Pp')
'''
###############################################################################       
...
pfam_file =  'ppa_aa_conekt_edited.fa.tsv'
