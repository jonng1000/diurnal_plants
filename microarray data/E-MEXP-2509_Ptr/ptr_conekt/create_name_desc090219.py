...
df = pd.read_csv('Ptrichocarpa_444_v3.1.annotation_info.txt', delimiter="\t", index_col=0)
...
# file.
record_dict = SeqIO.to_dict(SeqIO.parse("Ptrichocarpa_444_v3.1.cds_primaryTranscriptOnly.fa", "fasta"))
...
with open(gene_desc_file, 'w', newline='') as new_desc:
    ...
    for key in record_dict.keys():
        ...
        # allows script to handle IDs with 1 or 2 dots correctly
        if key.count('.') == 1:
            name = key.split('.')[0]
        elif key.count('.') == 2:
