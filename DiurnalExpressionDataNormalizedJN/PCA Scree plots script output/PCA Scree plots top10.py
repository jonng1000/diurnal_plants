...
import csv
'''
This script finds all .txt files in the working directory, and performs PCA
...
def PCA_analysis(input_file, file_name, output_file):
    '''
    This function runs the codes to generate a Scree and PCA plot, and a .txt
    file containing top 10 genes with the biggest loading scores to PC1
    '''
    ###############################################################################
    ...
    # need to put this everytime I plot sometihng, so that the previous figure
    # is cleared from memory and does not overlap with the next one,
    plt.close() 
    ###############################################################################
    # creates a PCA plot
    ###############################################################################
...
for file in os.listdir('./'):
    if file.endswith('.txt') and 'top10' not in file:
        input_file = file
        file_name = file.split('.')[0]
        output_file = file_name + '_top10' + '.txt'
        PCA_analysis(input_file, file_name, output_file)
