...
# creates list_timepoints, which stores day/night timepoint information e.g.
# L1, L3, L5....
list_timepoints = []
gene_ex_df_col_names = gene_ex_df.columns

timepoints = []
for t in gene_ex_df_col_names:
    timepoints.append(t.split()[0].split('.')[0])
...
for i, phase in enumerate(np.unique(day_night_phase)):
    ...
    sc = plt.scatter(phase_specific_PC1, phase_specific_PC2, marker=markers[i],
                     c=phase_specific_cseq, vmin=1, vmax=24, cmap='winter_r', s=400)
plt.colorbar(sc, ticks=[1, 5, 10, 15, 20, 24], orientation='horizontal')
